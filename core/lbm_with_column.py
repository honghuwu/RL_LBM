...
class lbm_solver:
    ...
    def collision(self):
        for i, j in ti.ndrange(self.nx, self.ny):
            ...
            for k in ti.static(range(9)):
                # 计算新的分布函数值
                new_f = (self.f_old[i,j][k] - 
                        self.inv_tau * (self.f_old[i,j][k] - feq_ij[k]) + vol_f[k])
                
                # 检查结果是否为有效值
                if ti.math.isnan(new_f) or ti.math.isinf(new_f):
                    new_f = feq_ij[k]  # 使用平衡分布函数作为备用值
                
                self.f_new[i,j][k] = new_f
    ...
    def update_macro_vars(self):
        ...
        for i, j in ti.ndrange(self.nx, self.ny):
            ...
            # 确保密度为正值且有限
            if new_rho <= 0.0 or ti.math.isnan(new_rho) or ti.math.isinf(new_rho):
                new_rho = 1.0  # 设置为默认密度值
                
            self.rho[i, j] = new_rho
            
            if new_rho > 1e-6:
                vel_candidate = momentum / new_rho
                # 检查速度是否为有效值
                if ti.math.isnan(vel_candidate.x) or ti.math.isnan(vel_candidate.y) or \
                   ti.math.isinf(vel_candidate.x) or ti.math.isinf(vel_candidate.y):
                    self.vel[i, j] = tm.vec2(0.0, 0.0)
                else:
                    self.vel[i, j] = vel_candidate
            else:
                self.vel[i, j] = tm.vec2(0.0, 0.0)
