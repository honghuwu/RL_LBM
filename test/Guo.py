...
class CylinderFlowSolver:
    ...
    def streaming(self):
        ...
        for i, j in ti.ndrange(self.nx, self.ny):
            for k in ti.static(range(9)):
                ...
                # 从源网格点拉取数据
                self.f_old[i, j][k] = self.f_final[ip, jp][k]
    ...
    def calculate_strouhal_number(self, min_samples=1000):
        ...
        try:
            # 修正：使用更合适的窗口大小和重叠
            nperseg = min(len(lift_data) // 8, 512)  # 更小的窗口，更好的频率分辨率
            ...
            frequencies, psd = signal.welch(
                ...
                noverlap=noverlap,
                window='hann',  # 使用汉宁窗
                detrend='linear'  # 线性去趋势
            )
            
            # 找到主频率（排除零频率和过高频率）
            # 限制频率范围，避免噪声影响
            valid_freq_mask = (frequencies > 0.001) & (frequencies < fs/4)
            
            if np.any(valid_freq_mask):
                psd_valid = psd[valid_freq_mask]
                freq_valid = frequencies[valid_freq_mask]
