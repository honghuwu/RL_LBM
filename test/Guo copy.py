...
class CylinderFlowSolver:
    def __init__(
        ...
        self.num_boundary_pts, x_cylinder_np, y_cylinder_np = obstacles_generate_cylinder(
            ...
            num_points = int(self.cylinder_radius * 2 * 3.141592653589793 * 2)
        )
    ...
    def apply_bc(self):
        """ 施加外围边界条件 - 周期性边界版本 """
        # 左边界：可选择使用Zou-He边界条件或简单边界条件
        # 如果遇到不稳定问题，可以切换到简单边界条件
        use_zou_he = False  # 设置为False可切换到简单边界条件
        
        if use_zou_he:
            # Zou-He边界条件（更精确但可能不稳定）
            self.apply_zou_he_left_boundary()
        else:
            # 简单边界条件（更稳定但精度较低）
            self.apply_simple_left_boundary()
    ...
    @ti.func
    def apply_simple_left_boundary(self):
        """
        简单左边界条件（备选方案）
        直接设置速度和密度，然后计算平衡态分布函数
        """
        i = 0  # 左边界位置
        for j in range(self.ny):
            # 设定边界条件
    ...
    @ti.func
    def apply_zou_he_left_boundary(self):
        """
        改进的Zou-He边界条件实现 - 左边界（西边界）
        给定速度边界条件：u = (inlet_velocity, 0)
        
        对于D2Q9格子，左边界需要重构的分布函数为：f1, f5, f8
        已知的分布函数为：f0, f2, f3, f4, f6, f7
        """
        i = 0  # 左边界位置
        for j in range(self.ny):
            # 设定边界速度
            u_boundary = self.inlet_velocity
            v_boundary = 0.0
            
            # 确保边界速度在合理范围内
            u_boundary = ti.max(ti.min(u_boundary, 0.3), 0.001)  # 限制在0.001到0.3之间
            
            # 从已知分布函数计算密度
            # 对于左边界，已知的分布函数：f0, f2, f3, f4, f6, f7
            # 未知的分布函数：f1, f5, f8
            
            # 计算已知分布函数的总和
            known_sum = (self.f_old[i, j][0] + self.f_old[i, j][2] + self.f_old[i, j][4] + 
                        self.f_old[i, j][3] + self.f_old[i, j][6] + self.f_old[i, j][7])
            
            # 计算向左流动的分布函数总和（f3, f6, f7）
            left_flow_sum = self.f_old[i, j][3] + self.f_old[i, j][6] + self.f_old[i, j][7]
            
            # 根据Zou-He边界条件计算密度
            # 修正公式：rho = (known_sum + 2*left_flow_sum) / (1 - u_boundary)
            denominator = 1.0 - u_boundary
            if ti.abs(denominator) < 1e-6:
                denominator = 1e-6  # 防止除零
            
            rho_boundary = (known_sum + 2.0 * left_flow_sum) / denominator
            
            # 确保密度在合理范围内
            rho_boundary = ti.max(ti.min(rho_boundary, 2.0), 0.5)
            
            # 更新宏观量
            self.rho[i, j] = rho_boundary
            self.vel[i, j] = tm.vec2(u_boundary, v_boundary)
            
            # 计算未知分布函数 - 使用更稳定的公式
            # f1 = f3 + (2/3) * rho * u
            self.f_old[i, j][1] = self.f_old[i, j][3] + (2.0/3.0) * rho_boundary * u_boundary
            
            # 计算横向速度差项
            transverse_diff = self.f_old[i, j][2] - self.f_old[i, j][4]
            
            # f5 = f7 - 0.5*(f2 - f4) + (1/6)*rho*u + 0.5*rho*v
            self.f_old[i, j][5] = (self.f_old[i, j][7] - 0.5 * transverse_diff + 
                                  (1.0/6.0) * rho_boundary * u_boundary + 0.5 * rho_boundary * v_boundary)
            
            # f8 = f6 + 0.5*(f2 - f4) + (1/6)*rho*u - 0.5*rho*v
            self.f_old[i, j][8] = (self.f_old[i, j][6] + 0.5 * transverse_diff + 
                                  (1.0/6.0) * rho_boundary * u_boundary - 0.5 * rho_boundary * v_boundary)
            
            # 确保分布函数为正值（数值稳定性）
            for k in ti.static([1, 5, 8]):
                self.f_old[i, j][k] = ti.max(self.f_old[i, j][k], 1e-12)
